...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const militanintitlesi = new TextInputBuilder()
            ...
            .setPlaceholder(
                "Embed içerisindeki title'da yani en üstünde ne yazmasını istiyorsun?"
            )
            .setMaxLength(15)
        ...
        const militanindesci = new TextInputBuilder()
            ...
            .setPlaceholder(
                "Embed içerisindeki mesaj'da yani içerisinde ne yazmasını istiyorsun?"
            )
        ...
        const minikinbirincisecenegi = new ActionRowBuilder().addComponents(
            militaninbirincisecenegi
        );
        const minikinikincisecenegi = new ActionRowBuilder().addComponents(
            militaninikincisecenegi
        );
        const minikinucuncusecenegi = new ActionRowBuilder().addComponents(
            militaninucuncusecenegi
        );
        const minikintitlesi = new ActionRowBuilder().addComponents(
            militanintitlesi
        );
        const minikindesci = new ActionRowBuilder().addComponents(
            militanindesci
        );
        militaninticketmodali.addComponents(
            minikintitlesi,
            minikindesci,
            minikinbirincisecenegi,
            minikinikincisecenegi,
            minikinucuncusecenegi
        );
        await interaction.showModal(militaninticketmodali);

        const filter = (i) => i.customId === "militaninticketbaslangicmodali";
        interaction
            .awaitModalSubmit({ filter, time: 90000 })
            .then(async (modalSubmitInteraction) => {
                const militanintitlesi =
                    modalSubmitInteraction.fields.getTextInputValue(
                        "militanintitlesisecenegi"
                    );
                const militanindesci =
                    modalSubmitInteraction.fields.getTextInputValue(
                        "militanindescisecenegi"
                    );
                const militaninbirincisecenegii =
                    modalSubmitInteraction.fields.getTextInputValue(
                        "militaninbirincisecenegi"
                    );
                const militaninikincisecenegii =
                    modalSubmitInteraction.fields.getTextInputValue(
                        "militaninikincisecenegi"
                    );
                const militaninucuncusecenegii =
                    modalSubmitInteraction.fields.getTextInputValue(
                        "militaninucuncusecenegi"
                    );
                ...
                const militaninmenusu = new ActionRowBuilder().addComponents(
                    new StringSelectMenuBuilder()
                        ...
                        .setPlaceholder(
                            minik.ticket.menuayarlari.menuplaceholder
                        )
                        .addOptions([
                            {
                                label: militaninbirincisecenegii,
                                emoji: minik.ticket.menuayarlari
                                    .birsecenekemoji,
                            ...
                            },
                            {
                                ...
                                description:
                                    "Menüdeki seçeneğinizi sıfırlarsınız.",
